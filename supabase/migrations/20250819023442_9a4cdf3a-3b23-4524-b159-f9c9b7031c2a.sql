...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    VALUES (NEW.user_id, 'appointment_reminder', ARRAY['in_app', 'email'], ARRAY[1440, 120, 30]);
    
    user_prefs.reminder_timing := ARRAY[1440, 120, 30];
    user_prefs.channels := ARRAY['in_app', 'email'];
  END IF;
  ...
  INSERT INTO public.notifications (
    ...
    now(),
    NEW.id,
    COALESCE((SELECT current_stage FROM public.patient_journey_stages WHERE user_id = NEW.user_id), 'screening'),
    2
  );
...
('screening_results', 'post_screening', 'Your Screening Results are Ready', 'Your mammogram results are ready. Please schedule your follow-up appointment with Dr. {{doctor_name}}. Click here to book.', 'in_app'),
('screening_results', 'post_screening', 'Follow-up Required', 'Next step: a diagnostic ultrasound/biopsy has been recommended. Here''s what that means → {{education_link}}.', 'in_app'),
('diagnosis_available', 'diagnosis', 'Pathology Report Available', 'Your pathology report is available. Please meet with your care team to discuss treatment options.', 'in_app'),
('treatment_reminder', 'chemotherapy', 'Chemo Session Reminder', 'Chemo Session #{{session_number}} is tomorrow at {{time}}. Don''t forget to bring your ID, snacks, and a support person if possible.', 'in_app'),
('post_treatment', 'chemotherapy', 'Post-Chemo Check-in', 'How are you feeling today? Tap here to record symptoms and get support.', 'in_app'),
('pre_treatment', 'surgery', 'Surgery Reminder', 'Surgery scheduled for {{date}} at {{time}}. Please avoid food after midnight and arrange transportation.', 'in_app'),
('post_treatment', 'surgery', 'Post-Surgery Care', 'It''s Day {{days_post_surgery}} after surgery. Remember to check your wound site for signs of infection. Learn what''s normal and when to seek help.', 'in_app'),
('treatment_reminder', 'radiation', 'Radiation Session', 'Radiation therapy session today at {{time}}. Try to wear loose clothing.', 'in_app'),
('wellness_tip', 'radiation', 'Skin Care Tip', 'Skin care tip: Apply moisturizer after treatment, not before.', 'in_app'),
('support_group', 'survivorship', 'Support Group Tonight', 'Join tonight''s survivor support group on WhatsApp at 7 PM. Share your journey and hear from others.', 'in_app'),
...
SELECT DISTINCT user_id, 'screening'
